...
pub fn tokenize(lines: Vec::<String>) -> Vec<String>{
    ...
    for line in lines{
	//step1 split by white space
	let line_splited_ws = line.split_whitespace();
	for word in line_splited_ws{
	    //step2 split by symbol
	    tokens.extend_from_slice(&split_symbol(word));
	}
    }
